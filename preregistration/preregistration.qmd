---
title: "Multimodal Text Reading Comprehension: Preregistration"
format:
  pdf:
...
A longstanding line of research in computational psycholinguistics seeks to explain reading comprehension in terms of measurable text and reader variables. Rather than relying on end-to-end neural language models that learn statistical mappings directly from textual input to behavioural or comprehension outcomes, this body of work identifies specific linguistic features operating at the lexical (word-level), syntactic (sentence-structural) and discourse (cross-sentence coherence and organisation) levels of representation that predict how readers engage with and understand text.

At the lexical level, word frequency stands out as one of the most robust predictors of reading behaviour. High-frequency words are recognised faster and fixated for shorter durations than low-frequency words, a pattern replicated across dozens of eye-tracking studies and remarkably consistent across languages and orthographies [e.g., @meziereUsingEyetrackingMeasures2023; @normanEffectsContextualDiversity2025]. In children, frequency effects tend to be larger than in adults, reflecting the less automatised nature of early lexical access, in which word recognition depends more heavily on effortful lexical processing and accumulated reading experience [@hsiaoSemanticDiversityFrequency2018]. To operationalise lexical frequency reliably, researchers commonly use subtitle-based frequency norms such as the Subtitle Lexical Frequencies (SUBTLEX) databases, which closely approximate everyday language exposure across multiple languages [@brysbaertWordPrevalenceNorms2019]. 

A related lexical property, word concreteness, also shapes comprehension: concrete words, especially those with high imageability (e.g., *apple*, *river*), are processed more quickly and remembered better than abstract words (e.g., *justice*, *theory*) [@magnaboscoEyeSemanticsStudy2024]. This concreteness advantage is often attributed to dual-coding accounts, in which concrete words activate both verbal and imagistic representations [@bernabeuLanguageSensorimotorSimulation2022; @kaupInterplayModalAmodal2025]. The effect is especially pronounced in younger readers and second-language learners, who rely more heavily on grounded sensory representations to construct meaning from text [@murakiDevelopmentAbstractWord2025]. 

A third lexical variable, the diversity of vocabulary within a text, modulates reading difficulty in ways that depend on the reader's own vocabulary knowledge [@vandenboschRoleLinguisticDiversity2019a; @cainReadingComprehensionVocabulary2014]. For skilled adults, moderate lexical diversity may sustain engagement through varied and precise word choice; for less proficient readers, including children, high diversity can tax lexical processing resources and impair comprehension. Lexical diversity is typically operationalised through indices such as the type/token ratio (TTR) or the Measure of Textual Lexical Diversity (MTLD) [@fredrickLexicalDiversitySyntactic2025]. 

At the syntactic level, sentence structure exerts its own demands on the reader. Measures such as mean dependency distance (the average linear distance between syntactically related words), the number of embedded clauses (clauses nested within other clauses), and subordination ratios (the proportion of clauses that depend on a main clause) all capture aspects of structural complexity that predict reading difficulty [@nielsenSyntacticComprehensionSeparate2025a]. Longer dependency distances, in particular, impose greater working memory demands, as readers must maintain information across intervening material before resolving a syntactic relationship. Because working memory capacity is still developing in children, they are disproportionately affected by syntactic complexity relative to adults [@delageSyntaxWorkingMemory2019; @cuiRoleExecutiveFunctions2024a; @changInfluencesExecutiveFunction2020a] — a developmental asymmetry that is central to several of the hypotheses we test in this study.

Beyond words and sentences, the coherence of a text — the degree to which successive ideas are logically and semantically connected — plays a critical role in comprehension. Computational measures of coherence include latent semantic analysis (LSA)-based sentence-to-sentence similarity, co-reference overlap between adjacent sentences, and connective density — the frequency of explicit linking words. Together, these measures index the ease with which readers can integrate information across a passage [@crossleyToolAutomaticAnalysis2019]. Highly coherent texts reduce the inferential burden and are generally associated with better comprehension, particularly for readers with lower prior knowledge [@hattanExpandingScienceReading2024]. Complementing these deeper semantic relations are surface-level cohesion signals: explicit connectives such as *because*, *however* and *therefore* that mark the logical structure of a text. Automated tools such as Coh-Metrix [@graesserCohmetrixMeasuresText2014] now make it possible to compute a comprehensive suite of coherence and cohesion indices, enabling large-scale analyses of text difficulty that would be impractical to conduct by hand [@crossleyDevelopingLinguisticConstructs2025].

These individual variables have been combined into composite indices of text difficulty. Classical readability formulae (e.g., Flesch–Kincaid, Dale–Chall) aggregate surface-level features such as word length and sentence length into single scores, and although they are limited in linguistic sophistication, they remain widely used in educational and applied contexts [@crossleyAssessingReadabilityFormulas2023; @wangUtilizingMachineLearning2024]. More recent multi-dimensional frameworks integrate frequency, concreteness, syntactic complexity, and coherence into richer characterisations of what makes a text difficult and for whom, moving beyond simple heuristics toward discourse-level representations [@crossleyAssessingReadabilityFormulas2023; @goldmanReadingComplexImplications2024a; @wangUtilizingMachineLearning2024; @liExploringSyntacticComplexity2025; @crossleyDevelopingLinguisticConstructs2025].

Taken together, this body of research establishes that reading comprehension is jointly shaped by a constellation of text-level variables operating across multiple linguistic levels [@nationChildrensReadingDifficulties2019; @duffEstimatesIndividualDifferences2020a]. These variables predict reading behaviour — including reading time, fixation patterns, and comprehension accuracy — in both children and adults, with developmental differences in magnitude and direction. The present study builds on this body of work by incorporating these established predictors as fixed effects in our analytic models (see Analytic Plan), while also examining interactions with child age as a proxy for developing linguistic and cognitive capacity. We turn next to a more recent line of work that complements these feature-based approaches with neural language models.
...
In adults, evidence for this approach has accumulated rapidly. @hahnModelingTaskEffects2023 introduced a neural attention model that adapts its processing to task demands and reproduces adult eye-movement patterns, including fixation durations and skip rates, across different reading goals within a single architecture. The findings suggest that task-specific reading strategies may emerge from optimal attentional adaptations rather than requiring separate mechanisms. @hosseiniArtificialNeuralNetwork2024 demonstrated that GPT-2–style language models trained on a developmentally plausible corpus (~100M words) could predict adult functional magnetic resonance imaging (fMRI) responses to sentences with near-maximal fidelity, with performance saturating after moderate training data. Together, these findings suggest that adult reading is largely driven by core linguistic mechanisms — word predictability, sequential dependency processing — that current LLMs can capture with surprising accuracy.

However, do these models that characterise adult reading also reflect the mechanisms underlying language acquisition and literacy development in children? Reviewing computational models of reading development, @monaghanLiteracyEarlyLanguage2023 argues that literacy acquisition builds on earlier oral language representations, with models increasingly able to explain how interactions between orthography, phonology, and semantics shape reading development across languages. Empirical work has begun to complement these simulations with large-scale behavioural data. @sortkaerBothSidesStory2021, for instance, analysed logs from approximately 48,000 Danish schoolchildren using a reading app (BookBites) and found a moderate positive correlation (*r* ≈ 0.4) between app-derived reading speed and standardised reading test scores, suggesting that naturalistic digital reading behaviour captures some variance in developing literacy skills. More recently, interactive eye-tracking benchmarks such as EyeBench and OneStop have provided large-scale datasets linking gaze patterns to reading comprehension, opening the door to models that predict individual comprehension from rich behavioural signals.

Systematic reviews of the applied landscape further contextualise these developments. @bootonImpactMobileApplication2023a found that among e-book app features, supports directly tied to the text itself — such as in-built narration (read-aloud) — substantially improve pre-readers' story comprehension and vocabulary. In contrast, interactive features unrelated to linguistic content, such as embedded games, provide no measurable benefit. @chuangSystematicReviewEffectiveness2023 reached a similar conclusion after reviewing 50 studies of interactive children's books: supportive multimedia features (e.g., animations or contextual glossaries that reinforce story meaning) can enhance emergent literacy, whereas attentionally competing game-like elements may impede reading outcomes. These findings highlight that children's reading behaviour is shaped not only by linguistic processing demands, but also by how digital environments allocate attention during reading — a consideration that is central to the present study's design.

In sum, neural language models offer a powerful complement to traditional text-feature approaches: whereas hand-crafted variables capture theoretically interpretable sources of variation, LLM-derived measures appear sensitive to subtler contextual and distributional regularities [@seidenbergImpactLanguageExperience2018]. Yet it remains unclear whether the representations that characterise adult reading also capture the mechanisms underlying language acquisition and literacy development in children. Compared with adults, the empirical evidence in developmental populations remains limited, and established lexical, syntactic, and discourse level linguistic predictors may explain a greater proportion of variance in children's reading behaviour than model-derived representations. Our study is positioned at this intersection, using both established linguistic features and model-derived metrics to predict children's reading behaviour across the 7–10 age range. @tbl-adults-children summarises the key studies from both traditions that inform our hypotheses.
...
Despite the rich data that digital reading platforms can provide [@altamuraHowDigitalReading2025; @furenesComparisonChildrensReading2021a], remarkably few computational studies have exploited app-generated behavioural logs as a window into reading processes. The work that does exist falls broadly into two categories, which differ in their origins and scientific utility.
...
|---|---|---|
| @bootonImpactMobileApplication2023a | Educational e-books; children 3–11 yrs (11 studies) | App features: narration, augmented reality (AR), prompts, hotspots. Narration improved comprehension for pre-readers; AR boosted L2 vocabulary; hotspots showed no benefit. |
...
Should eye-tracking data become available for a subset of participants, we will model fixation durations and regression rates as additional outcome variables using the same predictor structure as the confirmatory models. Finally, we will explore whether text complexity features show non-linear (quadratic) relationships with reading time, which would suggest diminishing or accelerating effects at extreme levels of complexity.
